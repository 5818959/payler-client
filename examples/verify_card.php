...
$card = [
    'card_number' => PAYLER_TEST_CARD_6_NUMBER,
    'card_holder' => PAYLER_TEST_CARD_6_HOLDER,
    'expired_year' => PAYLER_TEST_CARD_6_YEAR,
    'expired_month' => PAYLER_TEST_CARD_6_MONTH,
    'cvv' => PAYLER_TEST_CARD_6_CVV,
];
...
if (PaymentStatus::AUTHORIZED == $response->status) {
    if (!isset($response->card_id) || empty($response->card_id)) {
        $advStatus = $client->getAdvancedStatus($orderId);
        $response->card_id = $advStatus->card_id ?? null;
    }
